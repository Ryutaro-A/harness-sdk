...
The config targets `bedrock-mantle` by default. In Python, set `endpoint` to `runtime`
to use `bedrock-runtime` instead, which is where the OpenAI GPT models are served
through [cross-Region inference](https://docs.aws.amazon.com/bedrock/latest/userguide/cross-region-inference.html)
...
model = OpenAIResponsesModel(
    ...
    bedrock_mantle_config={"endpoint": "runtime", "region": "us-east-1"},
)
